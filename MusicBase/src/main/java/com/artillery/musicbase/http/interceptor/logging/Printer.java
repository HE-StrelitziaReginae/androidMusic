package com.artillery.musicbase.http.interceptor.logging;
...
class Printer {
    ...
    static void printJsonRequest(LoggingInterceptor.Builder builder, Request request) {
        ...
        if (builder.getLogger() == null) I.log(builder.getType(), tag, REQUEST_UP_LINE);
        ...
        if (builder.getLogger() == null) I.log(builder.getType(), tag, END_LINE);
    }
    ...
    static void printJsonResponse(LoggingInterceptor.Builder builder, long chainMs, boolean isSuccessful, int code, String headers, String bodyString, List<String> segments) {
        ...
        if (builder.getLogger() == null) I.log(builder.getType(), tag, RESPONSE_UP_LINE);
    ...
    private static String[] getRequest(Request request, Level level) {
        ...
        message = METHOD_TAG + request.method() + DOUBLE_SEPARATOR + (isEmpty(header) ? "" : loggableHeader ? HEADERS_TAG + LINE_SEPARATOR + dotHeaders(header) : "");
    ...
    private static String[] getResponse(String header, long tookMs, int code, boolean isSuccessful, Level level, List<String> segments) {
        ...
        message = ((!TextUtils.isEmpty(segmentString) ? segmentString + " - " : "") + "is success : " + isSuccessful + " - " + RECEIVED_TAG + tookMs + "ms" + DOUBLE_SEPARATOR + STATUS_CODE_TAG + code + DOUBLE_SEPARATOR + (isEmpty(header) ? "" : loggableHeader ? HEADERS_TAG + LINE_SEPARATOR + dotHeaders(header) : ""));
        return message.split(LINE_SEPARATOR);
    }
